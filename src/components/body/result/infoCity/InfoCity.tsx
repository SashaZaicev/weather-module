...
type InfoCityPropsType = {
  city: any,
  removeSearchCity: (cityId: string) => void
}
export const InfoCity: FC<InfoCityPropsType> = React.memo(({
  ...
  const weatherDescription = city.weather.map((weatherEl: any, index: number) => {
  ...
  return (
    <div className={style.infoCityContainer}>
      ...
      <div className={style.firstRow}> {nameCity},{sysCountry} <IconButton
